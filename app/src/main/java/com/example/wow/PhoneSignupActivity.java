...
import com.example.wow.helper.Validator;
import com.example.wow.model.Profile;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        callbacks = new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
            ...
            public void onVerificationFailed(@NonNull FirebaseException e) {
                      dialog.dismiss();
                Toast.makeText(PhoneSignupActivity.this, getString(R.string.vafication), Toast.LENGTH_SHORT).show();
                setVisibleVisibility(llPhone.getId());
            }
            public void onCodeSent(@NonNull String verificationId,
                                   @NonNull PhoneAuthProvider.ForceResendingToken token) {
                super.onCodeSent(verificationId, token);
                Toast.makeText(PhoneSignupActivity.this, getString(R.string.raj), Toast.LENGTH_SHORT).show();
    ...
    private void signUp (){
        PhoneAuthOptions options =
                PhoneAuthOptions.newBuilder(mAuth)
                        .setPhoneNumber(edtPhone.getText().toString())
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful())  {
                          ...
                          // User user = new User(id, username, edtPhone.getText().toString(), "");
                            User user = new User(id, username, edtPhone.toString().toString().toString(),"");
 ...
 */
    ...
    private void writeNewProfile(Profile profile) {
        ...
        db.collection("profile").document(profile.getUserId())
                .set(userValues)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
    ...
    private void handleBtnPhoneClick() {
        ...
        if(!validator.isValidPhone(phone)) {
            Toast.makeText(this, getString(R.string.raj), Toast.LENGTH_SHORT).show();
        } else {
            dialog.show();
